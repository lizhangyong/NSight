...
local redis_c   = require "lua.resty.redis"
local json      = require(require("ffi").os=="Windows" and "lua.resty.dkjson" or "cjson")
...
function _M.connect_mod( self, redis )
    local sys_conf   = common.fetch_from_shared_cache(SYS_SHARED_NAME, KEY_CONFIG_RULE)
    local redis_addr = common.split_no_pat(json.decode(sys_conf).redis, ":")
    return redis:connect(redis_addr[1] or "127.0.0.1", redis_addr[2] or "6379")
end
...
function _M.commit_pipeline( self )
    return self:limit_paralles(self._commit_pipeline, self)
end
